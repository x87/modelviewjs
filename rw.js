...
function rwSetHierarchyRoot(frame, root) {
  frame.root = root;
  for (frame = frame.child; frame != null; frame = frame.next) rwSetHierarchyRoot(frame, root);
}
...
function rwFrameSynchLTM(f) {
  if (f.parent == null) mat4.copy(f.ltm, f.matrix);
  else mat4.multiply(f.ltm, f.matrix, f.parent.ltm);
...
function RpGeometryCreate(flags, numMorphTargets) {
  ...
  geo.texCoords = [];
  if (numTexCoords > 0) while (numTexCoords--) geo.texCoords.push([]);

  if (flags & 0x08) geo.prelit = [];

  while (numMorphTargets--) {
    let mt = { vertices: [] };
    if (flags & 0x10) mt.normals = [];
...
function RpAtomicRender(atomic) {
  if (atomic.geometry.instData === undefined) instanceGeo(atomic.geometry);
...
function RpClumpSetFrame(c, f) {
  c.frame = f;
}

function RpClumpRender(clump) {
  for (let i = 0; i < clump.atomics.length; i++) RpAtomicRender(clump.atomics[i]);
}
...
function instanceGeo(geo) {
  ...
  if (geo.meshtype == 1) header.prim = gl.TRIANGLE_STRIP;
...
function instanceVertices(geo) {
  ...
  for (i = 0; i < attribs.length; i++) attribs[i].stride = stride;
...
function rwFrameInit(f) {
  ...
  f.objects = [];
  m = f.matrix;
  f.matrix = mat4.fromValues(m[0], m[1], m[2], 0, m[3], m[4], m[5], 0, m[6], m[7], m[8], 0, m[9], m[10], m[11], 1);
  f.ltm = mat4.create();
  mat4.copy(f.ltm, f.matrix);
}

function rpMaterialInit(m) {
  if (m.texture) m.texture = RwTextureRead(m.texture.name, m.texture.mask);
  if (m.matfx && m.matfx.envTex) m.matfx.envTex = RwTextureRead(m.matfx.envTex.name, m.matfx.envTex.mask);
  if (m.specMat) m.specMat.texture = RwTextureRead(m.specMat.texture, "");
}
...
function rpGeometryInit(g) {
  ...
  for (let i = 0; i < g.meshes.length; i++) g.totalMeshIndices += g.meshes[i].indices.length;
}
