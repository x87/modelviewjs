...
function RwStreamReadString(stream, length) {
  ...
  let s = "";
  for (let i = 0; i < length && a[i] != 0; i++) s += String.fromCharCode(a[i]);
...
function rwStreamReadChunkHeader(stream) {
  ...
  if ((id & 0xffff0000) == 0) {
    ...
    version = (((id >> 14) & 0x3ff00) + 0x30000) | ((id >> 16) & 3);
    build = id & 0xffff;
  }
  return {
    type: t,
...
function rwPluginRegistryReadDataChunks(tklist, stream, object) {
  let header;
  if ((header = RwStreamFindChunk(stream, rwID_EXTENSION)) == null) return null;
  ...
  while (stream.offset < end) {
    ...
    if (header.type in tklist && tklist[header.type].streamRead) {
      if (!tklist[header.type].streamRead(stream, object, header.length)) return null;
    } else RwStreamSkip(stream, header.length);
  }
  return 1;
}
...
function rwStringStreamFindAndRead(stream) {
  let header;
  if ((header = RwStreamFindChunk(stream, rwID_STRING)) == null) return null;
  return RwStreamReadString(stream, header.length);
}

function rwFrameListStreamRead(stream) {
  let header;
  if ((header = RwStreamFindChunk(stream, rwID_STRUCT)) == null) return null;
  ...
  for (let i = 0; i < numFrames; i++) {
    ...
    frame = RwFrameCreate();
    mat4.set(frame.matrix, xx, xy, xz, 0, yx, yy, yz, 0, zx, zy, zz, 0, wx, wy, wz, 1);
    frames.push(frame);
    let parent = RwStreamReadInt32(stream);
    RwStreamReadInt32(stream); // unused
    if (parent >= 0) RwFrameAddChild(frames[parent], frame);
  }
  for (let i = 0; i < numFrames; i++) if (!rwPluginRegistryReadDataChunks(frameTKList, stream, frames[i])) return null;
  return frames;
}
...
function RwTextureStreamRead(stream) {
  ...
  if (!rwPluginRegistryReadDataChunks(textureTKList, stream, tex)) return null;
  return tex;
}
...
function RpMaterialStreamRead(stream) {
  ...
  if (textured) {
    if ((header = RwStreamFindChunk(stream, rwID_TEXTURE)) == null) return null;
    mat.texture = RwTextureStreamRead(stream);
    if (mat.texture == null) return null;
  }
  if (!rwPluginRegistryReadDataChunks(materialTKList, stream, mat)) return null;
  return mat;
}
...
function rpMaterialListStreamRead(stream) {
  ...
  let indices = [];
  while (numMaterials--) indices.push(RwStreamReadInt32(stream));
  let materials = [];
  for (let i = 0; i < indices.length; i++) {
    if (indices[i] >= 0) materials.push(materials[indices[i]]);
    else {
      if ((header = RwStreamFindChunk(stream, rwID_MATERIAL)) == null) return null;
...
function RpGeometryStreamRead(stream) {
  ...
  if (header.version < 0x34000) RwStreamSkip(stream, 12);
  ...
  if ((header = RwStreamFindChunk(stream, rwID_MATLIST)) == null) return null;
  geo.materials = rpMaterialListStreamRead(stream);
  if (geo.materials == null) return null;

  if (!rwPluginRegistryReadDataChunks(geometryTKList, stream, geo)) return null;
...
function rpMeshRead(stream, geo, length) {
  ...
  while (numMeshes--) {
    ...
    while (numIndices--) m.indices.push(RwStreamReadInt32(stream));
    geo.meshes.push(m);
  }
  return geo;
}
...
function rpGeometryListStreamRead(stream) {
  ...
  let geoms = [];
  while (numGeoms--) {
    if ((header = RwStreamFindChunk(stream, rwID_GEOMETRY)) == null) return null;
...
function rpClumpAtomicStreamRead(stream, frames, geos) {
  ...
  if (!rwPluginRegistryReadDataChunks(atomicTKList, stream, atomic)) return null;
...
function RpClumpStreamRead(stream) {
  ...
  if ((header = RwStreamFindChunk(stream, rwID_FRAMELIST)) == null) return null;
  ...
  if ((header = RwStreamFindChunk(stream, rwID_GEOMETRYLIST)) == null) return null;
  geos = rpGeometryListStreamRead(stream);
  if (geos == null) return null;

  while (numAtomics--) {
    if ((header = RwStreamFindChunk(stream, rwID_ATOMIC)) == null) return null;
    let a = rpClumpAtomicStreamRead(stream, frames, geos);
    if (a == null) return null;
    clump.atomics.push(a);
  }

  if (!rwPluginRegistryReadDataChunks(clumpTKList, stream, clump)) return null;
    ...
    switch (type) {
      case rpMATFXEFFECTBUMPMAP:
        ...
        if (RwStreamReadInt32(stream)) {
          ...
          if (mfx.bumpedTex == null) return null;
        }
        if (RwStreamReadInt32(stream)) {
          if ((header = RwStreamFindChunk(stream, rwID_TEXTURE)) == null) return null;
          mfx.bumpTex = RwTextureStreamRead(stream);
          if (mfx.bumpTex == null) return null;
        }
        break;
      case rpMATFXEFFECTENVMAP:
        ...
        if (RwStreamReadInt32(stream)) {
          ...
          if (mfx.envTex == null) return null;
        }
        break;
      case rpMATFXEFFECTDUAL:
        ...
        if (RwStreamReadInt32(stream)) {
          ...
          if (mfx.dualTex == null) return null;
        }
        break;
    }
  }
...
function rpMatfxAtomicStreamRead(stream, atomic, length) {
  ...
  if (atomic.matfx) atomic.pipeline = matFXPipe;
  return atomic;
}
